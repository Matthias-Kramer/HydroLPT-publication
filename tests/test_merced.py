...
def test_merced_river_hecras_smoke(tmp_path):
    plan_hdf = os.environ.get("HYDROLPT_MERCED_PLAN_HDF")
    if not plan_hdf:
        raise unittest.SkipTest(
            "Set HYDROLPT_MERCED_PLAN_HDF to run the external Merced River smoke test."
        )

    plan_hdf_path = Path(plan_hdf)
    if not plan_hdf_path.exists():
        raise unittest.SkipTest(f"Merced River HEC-RAS plan file not found: {plan_hdf_path}")

    release_x = float(os.environ.get("HYDROLPT_MERCED_RELEASE_X", "2083030.0"))
    release_y = float(os.environ.get("HYDROLPT_MERCED_RELEASE_Y", "637376.0"))
